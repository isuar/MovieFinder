...
async function search() {
    ...
    try {
        const response = await $fetch(`https://www.omdbapi.com/?apikey=8e3f600b&s=${query.value}`);
...
<template>
    <div class="container mx-auto px-4 py-8">
        ...
        <div v-else-if="movies.length" class="py-4">
            <p class="text-gray-600 dark:text-gray-400 mb-6 text-center">Found {{ movies.length }} results for "{{ query }}"</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                ...
                >
                    <NuxtLink :to="{ name: 'movies-id', params: { id: movie.imdbID } }" class="block h-full">
                        ...
                        <div class="p-4">
                            <h3 class="font-bold text-lg mb-1 text-gray-800 dark:text-white truncate">{{ movie.Title }}</h3>
